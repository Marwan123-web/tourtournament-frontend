...
export default function MatchesPage() {
  ...
  const handleCreateMatch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNewMatch({ tournamentId: '', homeTeamId: '', awayTeamId: '', scheduledAt: '', round: '' });
      fetchMatches();
    } catch (error) {
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {matches.map((match) => (
          <div key={match.id} className="bg-white border border-gray-200 rounded-xl shadow-sm p-6 hover:shadow-md transition-shadow">
            ...
            <div className="space-y-4 mb-6">
              <div className="flex items-center justify-between">
                <span className="text-lg font-semibold text-gray-900">{match.homeTeam.name}</span>
                ...
                <div className="w-12 h-1 bg-gray-300 rounded-full">
                  {match.homeScore !== null && match.awayScore !== null && (
                    <div 
                      ...
                      style={{ width: `${(match.homeScore! / (match.homeScore! + match.awayScore!)) * 100}%` }}
                    />
                  )}
                </div>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-lg font-semibold text-gray-900">{match.awayTeam.name}</span>
          ...
          <div className="bg-white rounded-xl p-8 max-w-sm w-full">
            ...
            <div className="flex items-center justify-center space-x-8 mb-8">
              <div className="text-center">
                <label className="block text-sm font-medium text-gray-700 mb-2">{editingMatch.homeTeam.name}</label>
                <input
                  type="number"
                  min="0"
              ...
              <div className="text-center">
                <label className="block text-sm font-medium text-gray-700 mb-2">{editingMatch.awayTeam.name}</label>
                <input
                  type="number"
                  min="0"
                  className="w-20 p-3 border border-gray-300 rounded-lg text-center text-2xl font-bold"
            ...
            <div className="flex gap-3">
              <button
                onClick={() => {
                  const homeScore = (document.getElementById('homeScore') as HTMLInputElement).valueAsNumber;
                  const awayScore = (document.getElementById('awayScore') as HTMLInputElement).valueAsNumber;
